...
pub fn parse_line(line: &str, xlen: Xlen) -> Result<Instruction, String> {
    ...
    match mnem.as_str() {
        ...
        // CI-like: c.addi4spn rd, uimm  (rd' in x8..x15, base=sp)
        "c.addi4spn" => {
            if ops.len() != 2 { return Err("用法: c.addi4spn rd, uimm".into()); }
            let rd = parse_register(&ops[0])?;
            let uimm_val = parse_int(&ops[1])?;
            if uimm_val <= 0 { return Err("c.addi4spn 的 uimm 必须为正且非零".into()); }
            let u = uimm_val as u32;
            if (u & 0x3) != 0 { return Err("c.addi4spn 的 uimm 必须按 4 字节对齐".into()); }
            if u >= (1 << 10) { return Err("c.addi4spn 的 uimm 超出 10 位范围".into()); }
            let ciw = CIWType { rd, funct3: 0, uimm: Uimm::new(u, 10) };
            Ok(RVC::Caddi4spn(ciw).into())
        }
        // CI-like: c.addi rd, imm  (rd==rs1)
        "c.addi" => {
            if ops.len() != 2 { return Err("用法: c.addi rd, imm".into()); }
            let rd = parse_register(&ops[0])?;
            let imm = parse_int(&ops[1])?;
            let imm_bits = imm_signed_bits(imm, 6)?;
            let ci = CIType { rdrs1: rd, funct3: 0, imm: Imm::new(imm_bits, 6) };
            Ok(RVC::Caddi(ci).into())
        }
        // c.li rd, imm
        "c.li" => {
            if ops.len() != 2 { return Err("用法: c.li rd, imm".into()); }
            let rd = parse_register(&ops[0])?;
            let imm = parse_int(&ops[1])?;
            let imm_bits = imm_signed_bits(imm, 6)?;
            let ci = CIType { rdrs1: rd, funct3: 0, imm: Imm::new(imm_bits, 6) };
        ...
        // CL: c.lw rd, imm(rs1)  (rd', rs1' = x8..x15)
        "c.lw" => {
            if ops.len() != 2 { return Err("用法: c.lw rd, imm(rs1)".into()); }
            let rd = parse_register(&ops[0])?;
            let (imm_bits, rs1) = parse_mem_operand(&ops[1])?;
        ...
        // CS: c.sw rs2, imm(rs1)
        "c.sw" => {
            if ops.len() != 2 { return Err("用法: c.sw rs2, imm(rs1)".into()); }
            let rs2 = parse_register(&ops[0])?;
            let (imm_bits, rs1) = parse_mem_operand(&ops[1])?;
        ...
        "c.lwsp" => {
            if ops.len() != 2 { return Err("用法: c.lwsp rd, imm(sp)".into()); }
            let rd = parse_register(&ops[0])?;
            let (imm_bits, rs) = parse_mem_operand(&ops[1])?;
            if rs != 2 { return Err("c.lwsp 基址必须是 sp".into()); }
            let ci = CIType { rdrs1: rd, funct3: 0, imm: Imm::new(imm_bits, 8) };
            Ok(RVC::Clwsp(ci).into())
        }
        // C.SWSP: c.swsp rs2, imm(sp)
        "c.swsp" => {
            if ops.len() != 2 { return Err("用法: c.swsp rs2, imm(sp)".into()); }
            let rs2 = parse_register(&ops[0])?;
            let (imm_bits, rs) = parse_mem_operand(&ops[1])?;
            if rs != 2 { return Err("c.swsp 基址必须是 sp".into()); }
            let css = CSSType { rs2, funct3: 0, imm: Imm::new(imm_bits, 8) };
            Ok(RVC::Cswsp(css).into())
        }
        // RV64+: c.ld/c.sd/c.ldsp/c.sdsp
        "c.ld" => {
            if ops.len() != 2 { return Err("用法: c.ld rd, imm(rs1)".into()); }
            match xlen { Xlen::X64 | Xlen::X128 => {}, _ => return Err("c.ld 仅在 RV64/128 可用".into()) }
            let rd = parse_register(&ops[0])?;
            let (imm_bits, rs1) = parse_mem_operand(&ops[1])?;
        ...
        "c.sd" => {
            if ops.len() != 2 { return Err("用法: c.sd rs2, imm(rs1)".into()); }
            match xlen { Xlen::X64 | Xlen::X128 => {}, _ => return Err("c.sd 仅在 RV64/128 可用".into()) }
            let rs2 = parse_register(&ops[0])?;
            let (imm_bits, rs1) = parse_mem_operand(&ops[1])?;
        ...
        "c.ldsp" => {
            if ops.len() != 2 { return Err("用法: c.ldsp rd, imm(sp)".into()); }
            match xlen { Xlen::X64 | Xlen::X128 => {}, _ => return Err("c.ldsp 仅在 RV64/128 可用".into()) }
            let rd = parse_register(&ops[0])?;
            let (imm_bits, rs) = parse_mem_operand(&ops[1])?;
            if rs != 2 { return Err("c.ldsp 基址必须是 sp".into()); }
            let ci = CIType { rdrs1: rd, funct3: 0, imm: Imm::new(imm_bits, 9) };
            Ok(RVC::Cldsp(ci).into())
        }
        "c.sdsp" => {
            if ops.len() != 2 { return Err("用法: c.sdsp rs2, imm(sp)".into()); }
            match xlen { Xlen::X64 | Xlen::X128 => {}, _ => return Err("c.sdsp 仅在 RV64/128 可用".into()) }
            let rs2 = parse_register(&ops[0])?;
            let (imm_bits, rs) = parse_mem_operand(&ops[1])?;
            if rs != 2 { return Err("c.sdsp 基址必须是 sp".into()); }
            let css = CSSType { rs2, funct3: 0, imm: Imm::new(imm_bits, 9) };
            Ok(RVC::Csdsp(css).into())
        }
